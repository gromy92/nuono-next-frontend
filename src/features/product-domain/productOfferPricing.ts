export type ProductOfferPricingValidationIssue = {
...
export function collectProductOfferPricingValidationIssues(
  offer: Record<string, unknown> | undefined,
  label: string
): ProductOfferPricingValidationIssue[] {
  if (!offer) {
    return []
  }

  const issues: ProductOfferPricingValidationIssue[] = []
