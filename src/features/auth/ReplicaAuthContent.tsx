...
export function navigateAuthView(nextPath: string) {
...
export function ReplicaLoginContent({
  errorMessage,
  form,
  onInputChange,
  submitting,
...
export function ReplicaResetPwdContent() {
  return (
    <Form labelCol={{ span: 4 }} style={{ marginTop: 60 }}>
      ...
      <Form.Item label="密码"><Input.Password placeholder="请输入密码" /></Form.Item>
      <Form.Item label="确认密码"><Input.Password placeholder="请再次输入密码" /></Form.Item>
      <Button block shape="round" size="large" type="primary" disabled>确认</Button>
    </Form>
  );
}
