...
assert.equal(pageSource.includes("suite.suiteStatus === 'PENDING_REVIEW' || suite.suiteStatus === 'ADOPTED'"), true, '当前采用套图必须复用审核操作')
assert.equal(pageSource.includes('逐图修改意见'), true, '审核不通过必须支持按当前套图图片名称填写逐图意见')
assert.equal(pageSource.includes('整体意见'), true, '审核不通过必须支持填写整套统一意见')
assert.equal(pageSource.includes('请填写逐图修改意见或整体意见'), true, '逐图和整体意见必须至少填写一项')
assert.equal(pageSource.includes('buildReviewRejectPayload'), true, '逐图意见必须转换为对应图片返工请求')
