...
assert.doesNotMatch(tabSource, /resolvedProductActiveState\(item\) !== ['"]ACTIVE['"] \? renderEligibilityNotice/, 'inactive and unknown products must show their forecast evidence instead of an ineligibility placeholder')
assert.match(tabSource, /searchMatchedRows\.filter\(\(item\) => item\.calculationBlocked\)/, 'calculation blocking must depend on missing calculation evidence, not active state')
assert.match(tabSource, /在售状态待核实[\s\S]*不影响补货测算/, 'unknown active state must be informational and must not block replenishment calculation')
