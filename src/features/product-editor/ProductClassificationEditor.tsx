...
import { ProductCompetitorCategoryModal } from './ProductCompetitorCategoryModal';
import { ProductClassificationFields } from './ProductClassificationFields';
import { ProductDetailSection } from './ProductDetailSection';

function classificationTextValue(value: unknown) {
...
function optionValue(option: ProductClassificationOptionPayload) {
  return classificationTextValue(option.value || option.label).trim();
}
...
export function ProductClassificationEditor(props: {
  ...
  const storeCode = classificationTextValue(storeContext.storeCode);
  const brand = classificationTextValue(productSnapshotView?.identity.brand);
  const productFulltype = classificationTextValue(taxonomy.productFulltype);
  ...
  return (
    <>
      ...
      <ProductCompetitorCategoryModal
        open={categoryEditorOpen}
        materials={productCompetitorMaterials}
        officialFulltypeInput={renderFulltypeInput()}
        isOfficialFulltype={isOfficialNoonFulltypeCode}
        onUseFulltype={updateFulltype}
